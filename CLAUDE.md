# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## What this is

A **static website with one serverless endpoint**: a design-system template collection for 法考速记卷宗 ("Legal-Exam Cheat-Sheet Casefile"), a Chinese bar-exam study site. Each page renders one law-exam topic as a "casefile card" laid on a dark-desk background. The published site has **no build step** — pages are plain HTML files that each link one shared stylesheet (`tokens.css`); the only dynamic piece is the feedback board's Cloudflare Pages Function.

## Project layout

```
src/         the published site — one HTML page per topic + index/404/feedback,
             plus the shared tokens.css / terms.js / sidebar.js (kept flat: every
             page links them from the same folder, so URLs/links stay simple)
functions/   Cloudflare Pages Function — api/messages.js (must stay at repo root)
docs/        design spec (SKILL.md, components.md), CLOUDFLARE-SETUP.md, the .skill bundle
test/        node --test suites (design-system + messages API)
.github/     CI (lint + tests on every push/PR — does NOT deploy)
```

Linter configs (`.stylelintrc.json`, `.htmlvalidate.json`, `.prettierrc.json`,
`eslint.config.js`) stay at the root so the tools auto-discover them. Cloudflare Pages'
Git integration serves `src/` directly as the build output (with `functions/` auto-detected
at the repo root), so the **deployed URL of every page is `/<name>.html`** (e.g.
`/01-grid-overview.html`).

## Commands

There is nothing to build or compile. The site lives in `src/`. To preview, serve that folder (recommended, so the shared `tokens.css` resolves cleanly), or open an HTML file that sits alongside `tokens.css`:

```bash
cd src && python -m http.server 8000   # then visit http://localhost:8000
```

The feedback page (`src/feedback.html`) is a real shared message board backed by a Cloudflare Pages Function at `functions/api/messages.js` (route `/api/messages`) plus a KV namespace bound as `FEEDBACK_KV`. The frontend `fetch`es that endpoint (GET to list, POST to add). The homepage's visitor counter + 👍/👎 reactions use a second function `functions/api/stats.js` (route `/api/stats`) that **reuses the same `FEEDBACK_KV`** under a `site-stats` key (no extra binding). When any API is unreachable — opening the file as a bare `file://`, local `python -m http.server`, or a missing KV binding — the pages degrade gracefully (feedback → in-memory + banner; stats → `localStorage` counts, like animation still works); that is expected, not a bug. Deployment/binding steps are in `docs/CLOUDFLARE-SETUP.md`.

## Tests, linting & CI

Dev tooling only — the published site still has **no build step**. Needs Node 20+.

```bash
npm install      # one-time: install dev dependencies
npm run check    # lint (ESLint + Stylelint + html-validate + Prettier) + tests
npm test         # just the tests (node --test)
```

- `test/messages.test.js` unit-tests the `/api/messages` function against a fake KV
  (validation, 400/500 paths, trimming, the `匿名考生` default, the 500-item cap).
- `test/design-system.test.js` enforces the rules in this file as executable checks — every
  page links `tokens.css`, no page re-inlines the base layer, no orphan classes, the feedback
  XSS `textContent` guard, stamp `mix-blend-mode`, and `.dots` wiring. **If you change the
  system, these keep it honest.** When a linter complains about the deliberately compact
  style, tune the config (`.stylelintrc.json` / `.htmlvalidate.json` / `eslint.config.js`)
  rather than reformatting the pages.
- **CI** — `.github/workflows/ci.yml` runs `npm run check` on every push / PR. This is a
  **quality gate only — it does not deploy.**
- **Deploy** — handled by **Cloudflare Pages' Git integration**, not GitHub Actions. The repo
  is connected to a Pages project that auto-builds and deploys on every push to the production
  branch (build output dir `src`, `functions/` auto-detected; see `docs/CLOUDFLARE-SETUP.md`).
  There is no `deploy.yml` — Cloudflare's own pipeline would double-deploy alongside it.

## Authoritative design spec — read before editing any page

`docs/SKILL.md` is the design system's source of truth; `docs/components.md` holds the full copy-paste CSS/HTML for every component; `src/tokens.css` is the **shared stylesheet every page links** — the canonical runtime source for the tokens, base/desk layer, chrome, and all five layout archetypes. **Read `docs/SKILL.md` first** when creating or restyling any page — it defines hard rules that, if broken, mean the work no longer matches the system. The `.skill` file is just a zip bundle of those three docs for distribution; edit the loose files, not the archive.

## Architecture you can't see from the file list

- **Pages share one linked stylesheet.** Every page links `tokens.css` (`<link rel="stylesheet" href="tokens.css">`) — the single source of truth for the `:root` token set (`--desk`, `--folder`, `--ink`, `--brass`, `--seal`, `--indigo`, `--jade`, `--warn`, `--glow`, …), the base/desk layer, the shared chrome (topbar, hero, pagefoot/dots/footnav), and all five layout archetypes. A page's own inline `<style>` holds **only** its per-page `--accent` (plus rare overrides like `--glow`/`--shadow`), an optional `.page` width modifier (`.reading` 980 / `.narrow` 760 / `.doc` 620), and components unique to that one page. **Change a token or shared component once in `tokens.css` and it propagates to every page.** Trade-off (a deliberate decision): a page only renders with `tokens.css` beside it — fine when served or opened in-folder, but a lone `.html` emailed by itself will be unstyled.

- **Clickable term glossary (`terms.js` + `tokens.css` + per-page templates).** Inline `<button class="term" data-term="X">` elements open a frosted-glass overlay where the term flies to the left and a detail panel expands on the right. It's a reusable three-part split: styling lives in `tokens.css` (`.term`, `.term-overlay`, `.term-spread`, …), behaviour in the shared **`src/terms.js`** (linked like the stylesheet: `<script src="terms.js" defer></script>`, builds the overlay once + delegates clicks + runs the FLIP animation), and content in per-page `<template class="term-detail" data-term="X">` blocks the author writes. Matching is by `data-term` value (unique per page). The template HTML is author-trusted and injected via `cloneNode` — distinct from the feedback board's user-input XSS rule. Pages opt in by linking `terms.js`; `feedback.html`/`404.html`/etc. don't need it. Reference implementation: `src/susong-shixiao.html`. **A new shared `src/*.js` ships automatically** — Cloudflare serves `src/` directly, so any script beside the pages is published with them.

- **Quick-jump sidebar (`sidebar.js` + the `.sb-*` section of `tokens.css`).** Every page **except `index.html`** loads `<script src="sidebar.js" defer></script>` (the homepage doesn't — it *is* the catalog). The script injects a fixed left-edge "卷宗目录" handle (brass, same site-function semantics as `.fab-feedback`) plus a slide-out kraft-paper drawer listing every knowledge page grouped by subject family — group swatches use each family's accent, the current page is highlighted and its family auto-expanded, ESC/backdrop closes. Pages carry **zero sidebar HTML**; the whole site list lives in `SITE_MAP` at the top of `sidebar.js` as a **strict-JSON literal** between `SITE_MAP:BEGIN/:END` markers. `test/design-system.test.js` parses it and diffs it against the `index.html` catalog (families, hrefs, titles, order, accents) and every page's `data-subject` — adding a topic to one place but not the other fails `npm test`.

- **One topic = one page; name the file by its topic.** Each knowledge point is its own small HTML file named by a **pinyin topic slug** (e.g. `zhengdang-fangwei.html`), not by its layout — so the collection scales and URLs stay meaningful. **Do not group several topics into one file**: every page shares the cached `tokens.css`, so one-file-per-topic keeps each page tiny and fast (the reader downloads only the card they open). Five **layout archetypes** exist; pick the one matching the content shape and copy its current reference page rather than inventing a new layout:
  - card grid (many loosely-related points) — ref `susong-shixiao.html`
  - mindmap, pure-CSS stacked tree, no JS coordinate math (hierarchical concepts) — ref `gongtong-fanzui.html`
  - flowchart, animated "subway line" track (if/then decision chains) — ref `zhengdang-fangwei.html`
  - multi-column compare grid (2–3 confusable parallel concepts) — ref `xiaoli-santai.html`
  - mnemonic stamp + keyword cards (exact memorization) — ref `shanyi-qude.html`
  - Plus site pages: `index.html` (catalog), `404.html`, `feedback.html` (message board).

- **Subjects form per-subject series.** Each knowledge page declares its subject on `<body data-subject="刑事诉讼法">`. Pages are grouped into subject families (理论法 / 民法 / 刑法 / 刑事诉讼法 / 行政法 / 民事诉讼法 / 商法 / 经济法 / 知识产权法 / 国际法). Two families bundle several 法考 subjects under one accent/series: **理论法** (法治理论·法理学·宪法·法制史·职业道德) and **国际法/三国法** (国际公法·国际私法·国际经济法). The `.dots` progress bar shows **position within that family** (dot count = pages in the family; a 1-page subject shows a single dot), and prev/next in `.pager`/`.footnav` cycle **within the family** (the first page's "prev" and the last page's "next" point to `index.html`). The homepage catalog mirrors this: one `<section class="subj-group" data-subject="…">` per family. When adding a topic, append a dot to **every** page in that family and fix the neighbors' prev/next — the `test/design-system.test.js` "per-subject dots" test enforces it.

- **Color carries meaning, not decoration.** Each subject family owns one accent: 刑法 `--seal` red · 民法 `--indigo` blue · 刑事诉讼法 `#7a3b55` plum · 行政法 `#5d6b2f` olive · 理论法 `#46506b` slate · 民事诉讼法 `#2f6168` teal · 商法 `#684a7a` purple · 经济法 `#9a5a38` sienna · 知识产权法 `#2c6e84` cyan · 国际法 `#8f5066` mauve. A further new subject gets its **own** accent (never reuse an existing one); the homepage maps each to a `.subj-*` class. Outcome colors are fixed regardless of subject: `--jade` = valid/pass/good, `--seal` = invalid/warning conclusion, `--warn` = disputed/middle state. `--brass` is reserved for **site-function pages** (feedback, 404, homepage CTA). Don't apply an accent color that doesn't map to one of these meanings.

- **Per-page `--accent`** drives the topic's theme color; most components read `var(--accent)` so a page reskins by changing that one variable.

## Non-negotiable conventions

- Dark `--desk` background + light `--folder` (kraft-paper) cards — never inverted.
- `tokens.css` already provides `@media (prefers-reduced-motion: reduce)` and the base mobile breakpoints (760px / 480px) for the shared components; a page only adds breakpoints for its **own** unique components (collapse grids to one column, or allow horizontal scroll with a swipe hint).
- Stamp elements (`.seal`, `.stamp-sq`, `.stamp-reject`) require `mix-blend-mode: multiply` or they read as stickers, not ink stamps.
- No JS frameworks or build tooling in the shipped site — plain HTML/CSS + minimal vanilla JS where needed (e.g. the feedback board). (Dev-only test/lint tooling lives in `package.json` and never ships — see "Tests, linting & CI".)
- Any user-supplied content shown to other visitors must be rendered via `textContent`/DOM APIs, never string-concatenated into `innerHTML` (XSS).

## When adding a new topic page

Update the cross-page wiring that isn't auto-generated: link `tokens.css` **and `sidebar.js`** and keep the new page's inline `<style>` to just its `--accent` (plus any page-unique components) — never re-inline the shared base/chrome. Then add a `<span>` to the `.dots` progress indicator in **every** topic page (dot count = number of knowledge pages), fix the prev/next `.footnav` links on the neighboring pages, add a catalog card in `src/index.html` (the homepage search auto-includes any `.catalog .tcard`; add a `data-keywords="别名 法条 pinyin"` attribute to improve search recall), append the page to its subject family's entry in `SITE_MAP` (`src/sidebar.js` — the drawer sync tests fail otherwise), and — if you introduced a genuinely new layout — add a row to the layout table in `docs/SKILL.md`.

**SEO / social tags (per page, right after `<title>`).** Every page carries, inline in its `<head>`, a `<meta name="description">`, a `<link rel="icon" href="favicon.svg">`, an Open Graph block (`og:type`/`og:site_name`/`og:title`/`og:description`/`og:url`/`og:image`), and a `twitter:card` — these are static (scrapers like WeChat don't run JS) and **must be hand-added to each new page** (copy an existing topic page's block, swap the description/title/`og:url`). The shared OG image is `src/og-cover.png` (1200×630, regenerate via the brand card); the canonical base URL is `https://fkpass.pages.dev` (swap everywhere if a custom domain is set). Then regenerate `src/sitemap.xml`. `404.html` is the exception: it gets `<meta name="robots" content="noindex">` and **no** OG card (error pages aren't shared or indexed). `test/design-system.test.js` enforces all of this. The dev-only `scripts/check-head.mjs` (run via `npm run check:head`, part of `npm run check`) goes further: it asserts the **shared** head fragments — the font block, favicon, `og:image`/`og:image:width`/`og:image:height`, `og:locale`, `og:site_name`, `twitter:card`, `twitter:image` — are byte-identical across every page, so changing the canonical domain (or the font `<link>`) and missing a page fails CI instead of silently shipping a half-updated site. Per-page-unique tags (`title`/`description`/`og:title`/`og:description`/`og:url`/`canonical`/`og:type`) are intentionally not checked by it (their value differs by design).

**Sitemap is generated, not hand-edited.** `src/sitemap.xml` is produced by `scripts/gen-sitemap.mjs` from `git log` (each entry's `<lastmod>` = the date of the last commit that touched that HTML file), so it never drifts from reality. **When adding a topic page, run `npm run gen:sitemap`** to regenerate it; `npm run check:sitemap` (part of `npm run check`) fails CI if the checked-in file is out of sync. Homepage (`index.html`) is priority 1.0 and listed first, every other page 0.8, all `changefreq=weekly`; `404.html` is excluded (noindex). If the canonical domain changes, only `BASE` in the generator needs updating — re-run `npm run gen:sitemap`.

