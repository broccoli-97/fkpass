...
直接复制对应代码块到新页面里，改文字/数量即可。前提：页面已 `<link rel="stylesheet" href="tokens.css">`（其中已定义全部配色 token + 基础/公共组件样式），且已引入三种字体。**注意：本库列出的多数组件 CSS 已经在 `tokens.css` 里**，新页面通常无需再内联，这里的 CSS 仅供查阅、或做页面级微调时参考。
...
/* li 不能用 flex：条文常混排文本+<b>/<mark>，flex 会把每段拆成匿名 item 逐字竖排；悬挂缩进让整句自然换行 */
.cond-list li{position:relative; padding-left:29px; font-size:13.5px; line-height:1.6; color:var(--ink-soft);}
.cond-list .dot{position:absolute; left:0; top:1px; width:20px; height:20px; border-radius:50%; border:1.5px solid var(--brass);
